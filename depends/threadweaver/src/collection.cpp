...
public:
    ...
    void begin(JobPointer job, Thread *thread) override {
        ExecuteWrapper::begin(job, thread);
        Q_ASSERT(collection);
        collection->elementStarted(job, thread);
    }

    void end(JobPointer job, Thread *thread) override {
        Q_ASSERT(collection);
        collection->elementFinished(job, thread);
        ExecuteWrapper::end(job, thread);
    }

    void cleanup(JobPointer job, Thread *) override {
...
{
public:
    void begin(JobPointer, Thread *) override {
    }

    void end(JobPointer, Thread *) override {
    }
};
