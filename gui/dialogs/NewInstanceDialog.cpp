...
void NewInstanceDialog::setFromQuickMod(QuickModRef quickmod)
{
	...
	if (!MMC->qmdb()->someModMetadata(quickmod)->icon().isNull())
	{
		if (MMC->icons()->addIcon(quickmod.toString(), quickmod.userFacing(), MMC->qmdb()->someModMetadata(quickmod)->icon(), MMCIcon::FileBased))
		{
