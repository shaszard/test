...
void NewInstanceDialog::setFromQuickMod(QuickModRef quickmod)
{
	...
	if (!MMC->quickmodslist()->someModMetadata(quickmod)->icon().isNull())
	{
		if (MMC->icons()->addIcon(quickmod.toString(), quickmod.userFacing(), MMC->quickmodslist()->someModMetadata(quickmod)->icon(), MMCIcon::FileBased))
		{
